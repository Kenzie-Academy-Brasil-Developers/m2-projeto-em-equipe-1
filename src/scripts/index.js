...
import { requisicaoCadastrarUsuario, requisicaoFazerLogin } from "./requests.js"
...
if ( token ) window.location.replace( '/src/pages/home.html' );
...
function fazerCadastro() {
  ...
  btnFazerCadastro.addEventListener( "click", async ( event ) => {
    ...
    } else toast( "Dados inválidos", red )
  } )
}
...
export function fazerLogin() {
  ...
  btnLogin.addEventListener( "click", async ( event ) => {
    ...
    if ( requisicao.token ) {
      ...
      localStorage.setItem( "@KenziePets:Usuario", requisicao.token )
